...
func Test_client_redisPing(t *testing.T) {
	tests := []struct {
		...
		{
			clientOptions:  []clientoptions{withUsername("testuser"), withPassword("secret")},
			containerCmd:   []string{"redis-server", "--user", "default", "off", "--user", "testuser", "on", ">secret", "~*", "&*", "+@all"},
			containerImage: "redis:7.4",
			name:           "redis_acl",
			want:           "PONG",
			wantErr:        false,
		...
		{
			clientOptions:  []clientoptions{withUsername("testuser"), withPassword("secret")},
			containerCmd:   []string{"valkey-server", "--user", "default", "off", "--user", "testuser", "on", ">secret", "~*", "&*", "+@all"},
			containerImage: "valkey/valkey:7.2",
			name:           "valkey_acl",
			want:           "PONG",
			wantErr:        false,
